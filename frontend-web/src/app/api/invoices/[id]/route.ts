...
// Données mockées pour les factures (même tableau que dans route.ts)
...
export async function PUT(
  ...
  try {
    ...
    // Validation basique
    if (!invoiceData.invoice_number || !invoiceData.issue_date) {
      return NextResponse.json(
        { error: 'Le numéro de facture et la date d\'émission sont obligatoires' },
        { status: 400 }
      )
    }
